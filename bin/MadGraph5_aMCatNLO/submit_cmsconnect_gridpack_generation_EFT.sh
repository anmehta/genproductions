...
cat<<-EOF
	...
	transfer_input_files = $input_files, gridpack_generation_EFT.sh, /usr/bin/unzip
...
cat<<-EOF
	...
	unset CXX CC FC
	# Run
	iscmsconnect=1 bash -x gridpack_generation_EFT.sh "${card_name}" "${card_dir}" "${workqueue}" CODEGEN "${scram_arch}" "${cmssw_version}"
	exitcode=\$?
...
#iscmsconnect=1 bash -xe gridpack_generation_EFT.sh ${card_name} ${card_dir} ${workqueue} MADSPIN ${scram_arch} ${cmssw_version}
